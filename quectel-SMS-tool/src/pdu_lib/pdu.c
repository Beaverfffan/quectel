...
int pdu_encode(const char* service_center_number, const char* phone_number, const wchar_t* sms_text, unsigned char* output_buffer, int buffer_size) {
	if (buffer_size < 2)
		return -1;
	...
	// 5. SMS message.
	int sms_text_length = wcslen(sms_text);
    output_buffer[output_buffer_length++] = sms_text_length * 2;

    //printf("Length: %d\n", sms_text_length);

	for (int i = 0; i < sms_text_length; i++) {
        //printf("Decimal: %d\n", sms_text[i]);
        output_buffer[output_buffer_length++] = (sms_text[i] & 65280) >> 8;
        output_buffer[output_buffer_length++] = sms_text[i] & 255;
    }
/*
